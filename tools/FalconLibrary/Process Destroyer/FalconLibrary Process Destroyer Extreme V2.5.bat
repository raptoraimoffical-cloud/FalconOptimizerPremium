...
reg export "HKLM\System\CurrentControlSet\Services" "C:\FalconLibrary Tools\Process Destroyer\Revert\Services_Backup.reg" /y >nul 2>&1
if errorlevel 1 (
    echo [%DATE% %TIME%] PD Extreme Services Reg Backup: Failed to create. >> "C:\FalconLibrary Logs\FalconLibrary Log.txt"
) else ( 
    echo [%DATE% %TIME%] PD Extreme Services Reg Backup: Created successfully. >> "C:\FalconLibrary Logs\FalconLibrary Log.txt"
)

:: Creating TrustedInstaller Reg Backup.
reg export "HKLM\System\CurrentControlSet\Services\TrustedInstaller" "C:\FalconLibrary Tools\Process Destroyer\Revert\Trusted_Installer_Backup.reg" /y >nul 2>&1
if errorlevel 1 (
    echo [%DATE% %TIME%] PD Extreme TrustedInstaller Reg Backup: Failed to create. >> "C:\FalconLibrary Logs\FalconLibrary Log.txt"
) else ( 
    echo [%DATE% %TIME%] PD Extreme TrustedInstaller Reg Backup: Created successfully. >> "C:\FalconLibrary Logs\FalconLibrary Log.txt"
)

:: Creating Windows Installer Reg Backup.
reg export "HKLM\System\CurrentControlSet\Services\msiserver" "C:\FalconLibrary Tools\Process Destroyer\Revert\Windows_Installer_Backup.reg" /y >nul 2>&1
if errorlevel 1 (
    echo [%DATE% %TIME%] PD Extreme Windows Installer Reg Backup: Failed to create. >> "C:\FalconLibrary Logs\FalconLibrary Log.txt"
) else ( 
    echo [%DATE% %TIME%] PD Extreme Windows Installer Reg Backup: Created successfully. >> "C:\FalconLibrary Logs\FalconLibrary Log.txt"
)
...
for /L %%i in (1,1,239) do (
    ...
    if !errorlevel! equ 0 (
        set /A "FoundCount+=1"
        echo [%DATE% %TIME%] !FoundCount!: Found !svc!. >> "C:\FalconLibrary Logs\Extra\Process Destroyer Extreme Log.txt"
        echo ✔  Deleting !svc!.
        reg delete "HKLM\SYSTEM\CurrentControlSet\Services\!svc!" /f >nul 2>&1
        reg query "HKLM\SYSTEM\CurrentControlSet\Services\!svc!" >nul 2>&1
        if !errorlevel! equ 0 (
            set /A "DeleteCount+=1"
            echo [%DATE% %TIME%] !DeleteCount!: Failed to delete !svc!. >> "C:\FalconLibrary Logs\Extra\Process Destroyer Extreme Log.txt"
        ) else (
            set /A "DeleteCount+=1"
            echo [%DATE% %TIME%] !DeleteCount!: Successfully deleted !svc!. >> "C:\FalconLibrary Logs\Extra\Process Destroyer Extreme Log.txt"
        )
    ) else (
        set /A "NotFoundCount+=1"
        echo [%DATE% %TIME%] !NotFoundCount!: !svc! not found. >> "C:\FalconLibrary Logs\Extra\Process Destroyer Extreme Log.txt"
    )
)
