...
for /f "tokens=2 delims=:(" %%i in ('powercfg /list ^| findstr /C:"Falcon Ultimate Performance"') do (
    set Plan_Guid=%%i
)
for /f "tokens=2 delims=:(" %%i in ('powercfg /list ^| findstr /C:"Falcon Ultimate Performance Idle Off"') do (
