...
mkdir "%FalconLibrary_Backup_Folder%\Priority" >nul 2>&1
reg add "%regKeyGP%" /f >nul 2>&1
reg export "%regKeyGP%" "%FalconLibrary_Backup_Folder%\Priority\GraphicsPreferences.reg" /y >nul 2>&1
if errorlevel 1 (
) else ( 
)
reg export "%regKeyPR%" "%FalconLibrary_Backup_Folder%\Priority\Priority.reg" /y >nul 2>&1
if errorlevel 1 (
) else ( 
)
reg export "%regKeyFO%" "%FalconLibrary_Backup_Folder%\Priority\FSO.reg" /y >nul 2>&1
if errorlevel 1 (
) else ( 
)
...
color 9
chcp 65001 >nul 2>&1
"C:\FalconLibrary Tools\Edge Remover\setup.exe" --uninstall --system-level --force-uninstall >nul 2>&1
...
set "LockAppFound=0"
mkdir "%FalconLibrary_Backup_Folder%\LockApp" >nul 2>&1
if exist "%LockAppItem%" (
    set "LockAppFound=1"
    takeown /F "%LockAppItem%" >nul 2>&1
    icacls "%LockAppItem%" /grant administrators:F >nul 2>&1
    copy /Y "%LockAppItem%" "%FalconLibrary_Backup_Folder%\LockApp" >nul 2>&1
    if %ERRORLEVEL% equ 0 (
    ) else (
    )
...
set "BackupCount=0"
set "DeleteCount=0"
mkdir "%FalconLibrary_Backup_Folder%\Smartscreen" >nul 2>&1
for %%S in ("%SmartscreenItem1%" "%SmartscreenItem2%") do (
    if exist "%%~S" (
       ...
       copy /Y "%%~S" "%FalconLibrary_Backup_Folder%\Smartscreen" >nul 2>&1
       if !ERRORLEVEL! equ 0 (
       ) else (
       )
       del "%%~S" /s /f /q >nul 2>&1
...
set "BackupCount=0"
set "DeleteCount=0"
mkdir "%FalconLibrary_Backup_Folder%\Sync Programs" >nul 2>&1
for %%S in ("%SyncItem1%" "%SyncItem2%") do (
    if exist "%%~S" (
       ...
       takeown /F "%%~S" >nul 2>&1
       icacls "%%~S" /grant administrators:F >nul 2>&1
       copy /Y "%%~S" "%FalconLibrary_Backup_Folder%\Sync Programs" >nul 2>&1
       if !ERRORLEVEL! equ 0 (
       ) else (
       )
       del "%%~S" /s /f /q >nul 2>&1
...
set "BackupCount=0"
set "DeleteCount=0"
mkdir "%FalconLibrary_Backup_Folder%\Xbox Bloat" >nul 2>&1
for /L %%i in (1,1,22) do (
    ...
    if exist "!XboxfileToDelete!" (
        ...
        copy /Y "!XboxfileToDelete!" "%FalconLibrary_Backup_Folder%\Xbox Bloat" >nul 2>&1
        if %ERRORLEVEL% equ 0 (
